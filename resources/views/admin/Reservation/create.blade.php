...
@enderror
        <div class="mb-6">
            ...
            <input type="string" id="tel_number" name="tel_number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@flowbite.com" >
        </div>
        @error('email')
    <div class="alert alert-danger">{{ $message }}</div>
@enderror
...
<div class="mb-6">
    ...
    <input type="datetime-local" id="res_data" name="res_data" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="YYYY-MM-DD HH:MM" >
</div>
        @error('table_id')
    <div class="alert alert-danger">{{ $message }}</div>
@enderror
        <div class="mb-6">
         ...
         <option value="{{$tables->id}}">{{$tables->name}}(Guests {{$tables->gust_number}})</option>
          @endforeach
         </select>
          </div>
          @error('gust_number')
